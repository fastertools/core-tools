...
pub fn modulus(input: TwoNumberInput) -> ToolResponse {
    ...
    match logic::modulus_numbers(logic_input) {
        Ok(result) => {
...
#[cfg(feature = "library")]
pub fn modulus_pure(a: f64, b: f64) -> Result<f64, String> {
    if b == 0.0 {
        Err("Cannot calculate modulus with zero divisor".to_string())
    } else {
        Ok(a % b)
    }
}