...
pub fn add_numbers(input: TwoNumberInput) -> Result<ArithmeticResult, String> {
...
mod tests {
    ...
    fn test_positive_numbers() {
        ...
        let result = add_numbers(input).unwrap();
    ...
    fn test_large_numbers() {
        let input = TwoNumberInput { a: 1e10, b: 2e10 };
        let result = add_numbers(input).unwrap();
        assert_eq!(result.result, 3e10);
        assert_eq!(result.operation, "addition");
        assert_eq!(result.inputs, vec![1e10, 2e10]);
    }

    #[test]
    fn test_small_numbers() {
        let input = TwoNumberInput { a: 1e-10, b: 2e-10 };
        let result = add_numbers(input).unwrap();
        assert!((result.result - 3e-10).abs() < 1e-20);
        assert_eq!(result.operation, "addition");
        assert_eq!(result.inputs, vec![1e-10, 2e-10]);
    }
    ...
    fn test_decimal_precision() {
        let input = TwoNumberInput { a: 0.1, b: 0.2 };
        let result = add_numbers(input).unwrap();
        assert!((result.result - 0.3).abs() < 1e-15);
        assert_eq!(result.operation, "addition");
        assert_eq!(result.inputs, vec![0.1, 0.2]);
    }
}
