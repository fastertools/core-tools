...
pub fn validate_email(input: EmailValidatorInput) -> Result<EmailValidatorResult, String> {
    ...
    checks.no_edge_dots = !local.starts_with('.') && !local.ends_with('.') 
...
mod tests {
    ...
    fn test_invalid_emails() {
        let test_cases = vec![
            ...
            ("test@@example.com", "Email must contain exactly one @ symbol"),
            ("test..user@example.com", "Email cannot contain consecutive dots"),
            (".test@example.com", "Email parts cannot start or end with dots"),
            ("test.@example.com", "Email parts cannot start or end with dots"),
            ("test@.example.com", "Email parts cannot start or end with dots"),
        ...
        for (email, expected_error) in test_cases {
            ...
            assert!(actual_error.contains(expected_error), 
                    "Email '{}' should have error containing '{}', but got '{}'", email, expected_error, actual_error);
        }
    }
