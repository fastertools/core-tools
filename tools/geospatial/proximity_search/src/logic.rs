...
pub fn find_nearest_points(query_point: Point, candidate_points: Vec<Point>, max_results: Option<usize>, max_distance_meters: Option<f64>) -> Result<NearestPointsResult, String> {
...
mod tests {
    ...
    fn create_test_points() -> Vec<Point> {
        vec![
            Point { lat: 40.7128, lon: -74.0060, id: Some("NYC".to_string()) },      // New York
            Point { lat: 34.0522, lon: -118.2437, id: Some("LA".to_string()) },     // Los Angeles
            Point { lat: 41.8781, lon: -87.6298, id: Some("CHI".to_string()) },     // Chicago
            Point { lat: 29.7604, lon: -95.3698, id: Some("HOU".to_string()) },     // Houston
            Point { lat: 33.4484, lon: -112.0740, id: Some("PHX".to_string()) },    // Phoenix
        ]
    }

    #[test]
    fn test_proximity_search_basic() {
        let query_point = Point { lat: 40.7589, lon: -73.9851, id: Some("Times Square".to_string()) }; // Times Square
        ...
        for i in 1..result.nearest_points.len() {
            assert!(result.nearest_points[i-1].distance_meters <= result.nearest_points[i].distance_meters);
        }
    }

    #[test]
    fn test_proximity_search_with_max_results() {
        let query_point = Point { lat: 40.7589, lon: -73.9851, id: None };
        let candidates = create_test_points();
    ...
    fn test_haversine_distance_known_values() {
        let nyc = Point { lat: 40.7128, lon: -74.0060, id: None };
        let la = Point { lat: 34.0522, lon: -118.2437, id: None };
    ...
    fn test_haversine_distance_same_point() {
        let point = Point { lat: 40.7128, lon: -74.0060, id: None };
    ...
    fn test_calculate_bearing_same_point() {
        let point = Point { lat: 40.0, lon: -74.0, id: None };
    ...
    fn test_proximity_search_empty_candidates() {
        let query_point = Point { lat: 40.0, lon: -74.0, id: None };
        let candidates = vec![];
        
        let result = find_nearest_points(query_point, candidates, None, None);
        
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "At least one candidate point must be provided");
    }
    ...
    fn test_proximity_search_invalid_query_coordinates() {
        ...
        // Invalid latitude
        let invalid_query = Point { lat: 91.0, lon: -74.0, id: None };
        ...
        // Invalid longitude
        let invalid_query = Point { lat: 40.0, lon: 181.0, id: None };
        let result = find_nearest_points(invalid_query, candidates, None, None);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("Invalid query point longitude"));
    }
    ...
    fn test_proximity_search_nan_coordinates() {
        ...
        // NaN query point
        let nan_query = Point { lat: f64::NAN, lon: -74.0, id: None };
        let result = find_nearest_points(nan_query, candidates.clone(), None, None);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Query point latitude cannot be NaN or infinite");
        ...
        let result = find_nearest_points(query_point, candidates, None, None);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Candidate point longitude cannot be NaN or infinite");
    }
    ...
    fn test_proximity_search_infinite_coordinates() {
        ...
        let inf_query = Point { lat: f64::INFINITY, lon: -74.0, id: None };
        ...
        let result = find_nearest_points(query_point, candidates, None, None);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Candidate point latitude cannot be NaN or infinite");
    }
    ...
    fn test_proximity_search_invalid_max_distance() {
        ...
        let result = find_nearest_points(query_point.clone(), candidates.clone(), None, Some(-1000.0));
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Max distance must be positive and finite");
        
        // NaN max distance
        let result = find_nearest_points(query_point.clone(), candidates.clone(), None, Some(f64::NAN));
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Max distance must be positive and finite");
    ...
    fn test_proximity_search_boundary_coordinates() {
        ...
        let query_point = Point { lat: 90.0, lon: 180.0, id: None }; // North Pole, Date Line
        let candidates = vec![
            Point { lat: -90.0, lon: -180.0, id: Some("South Pole".to_string()) },
            Point { lat: 0.0, lon: 0.0, id: Some("Equator Prime".to_string()) },
        ];
    ...
    fn test_proximity_search_very_small_max_distance() {
        let query_point = Point { lat: 40.7128, lon: -74.0060, id: None }; // NYC
        let candidates = create_test_points();
    ...
    fn test_proximity_search_point_ids() {
        let query_point = Point { lat: 40.0, lon: -74.0, id: Some("Query".to_string()) };
        let candidates = create_test_points();
    ...
    fn test_proximity_search_crossing_date_line() {
        let query_point = Point { lat: 0.0, lon: 179.0, id: None }; // Near date line
        let candidates = vec![
            Point { lat: 0.0, lon: -179.0, id: Some("West of date line".to_string()) },
            Point { lat: 0.0, lon: 178.0, id: Some("East of query".to_string()) },
        ];
