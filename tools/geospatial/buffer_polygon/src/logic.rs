...
pub fn create_circular_buffer(center: Point, radius_meters: f64, num_points: Option<usize>) -> Result<BufferResult, String> {
...
mod tests {
    ...
    fn test_circular_buffer_basic() {
        let center = Point { lat: 40.7128, lon: -74.0060 }; // New York City
    ...
    fn test_circular_buffer_point_constraints() {
        let center = Point { lat: 45.0, lon: 0.0 };
        let radius = 1000.0;
    ...
    fn test_circular_buffer_poles() {
        let center = Point { lat: 89.0, lon: 0.0 }; // Near North Pole
        let radius = 1000.0;
        ...
        for point in result.buffer_polygon {
            assert!((point.lat - center.lat).abs() < 1.0, 
                    "Point latitude {} too far from center {}", point.lat, center.lat);
        }
    }

    #[test]
    fn test_circular_buffer_small_radius() {
        let center = Point { lat: 40.0, lon: -74.0 };
    ...
    fn test_circular_buffer_invalid_latitude() {
        let center = Point { lat: 91.0, lon: 0.0 }; // Invalid latitude
        let radius = 1000.0;
    ...
    fn test_circular_buffer_boundary_coordinates() {
        ...
        let center = Point { lat: 90.0, lon: 0.0 }; // North Pole
        let radius = 1000.0;
        let result = create_circular_buffer(center, radius, Some(8));
        assert!(result.is_ok());
        
        let center = Point { lat: -90.0, lon: 0.0 }; // South Pole
        let result = create_circular_buffer(center, radius, Some(8));
        assert!(result.is_ok());
        
        // Test boundary longitude values
        let center = Point { lat: 0.0, lon: 180.0 }; // Date line
        let result = create_circular_buffer(center, radius, Some(8));
        assert!(result.is_ok());
    ...
    fn test_circular_buffer_point_distribution() {
        ...
        for point in points {
            if point.lat > center.lat { has_north = true; }
            if point.lat < center.lat { has_south = true; }
            if point.lon > center.lon { has_east = true; }
            if point.lon < center.lon { has_west = true; }
        }
        
        assert!(has_north && has_south && has_east && has_west, 
