...
mod tests {
    ...
    fn test_polygon_area_basic_square() {
        ...
        assert!(
            (result.area_square_kilometers - result.area_square_meters / 1_000_000.0).abs() < 1.0
        );
    ...
    fn test_polygon_area_nan_coordinates() {
        ...
        assert_eq!(
            result.unwrap_err(),
            "Coordinate latitude cannot be NaN or infinite"
        );
        ...
        assert_eq!(
            result.unwrap_err(),
            "Coordinate longitude cannot be NaN or infinite"
        );
    }
