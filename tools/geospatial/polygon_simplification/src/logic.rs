...
pub fn polygon_simplification_logic(input: PolygonSimplificationInput) -> Result<PolygonSimplificationResult, String> {
    if input.polygon.len() < 3 {
        return Err("Polygon must have at least 3 vertices".to_string());
    }
    
    if input.tolerance_meters <= 0.0 || input.tolerance_meters.is_nan() || input.tolerance_meters.is_infinite() {
...
mod tests {
    ...
    fn create_complex_polygon() -> Vec<Point> {
        vec![
            Point { lat: 0.0, lon: 0.0 },
            Point { lat: 0.001, lon: 0.001 }, // Close to line
            Point { lat: 0.1, lon: 0.1 },
            Point { lat: 0.2, lon: 0.15 }, // Slight deviation
            Point { lat: 0.3, lon: 0.3 },
    ...
    fn test_haversine_distance_same_point() {
        let p1 = Point { lat: 40.7128, lon: -74.0060 };
        let p2 = Point { lat: 40.7128, lon: -74.0060 };
    ...
    fn test_polygon_simplification_invalid_algorithm() {
        ...
        let result = polygon_simplification_logic(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Algorithm must be 'douglas_peucker' or 'visvalingam'");
    }
    ...
    fn test_reduction_percentage_calculation() {
        ...
        let expected_percentage = ((result.original_vertex_count - result.simplified_vertex_count) as f64 
                                  / result.original_vertex_count as f64) * 100.0;
    ...
    fn test_algorithm_output_consistency() {
        ...
        assert_eq!(result.simplified_polygon.first(), result.original_polygon.first());
        assert_eq!(result.simplified_polygon.last(), result.original_polygon.last());
    }
}