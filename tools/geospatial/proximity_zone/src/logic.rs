...
pub fn proximity_zone_analysis(center: Point, radius_meters: f64, candidate_points: Vec<Point>) -> Result<ProximityZoneResult, String> {
...
mod tests {
    ...
    fn create_test_points_around_center() -> Vec<Point> {
        vec![
            Point { lat: 40.7128, lon: -74.0060, id: Some("Close1".to_string()) },     // ~0km from NYC
            Point { lat: 40.7228, lon: -74.0060, id: Some("Close2".to_string()) },     // ~1.1km north
            Point { lat: 40.7128, lon: -73.9960, id: Some("Close3".to_string()) },     // ~0.8km east
            Point { lat: 40.8000, lon: -74.0060, id: Some("Medium".to_string()) },     // ~9.7km north
            Point { lat: 41.0000, lon: -74.0060, id: Some("Far".to_string()) },       // ~32km north
        ]
    }

    #[test]
    fn test_proximity_zone_basic() {
        let center = Point { lat: 40.7128, lon: -74.0060, id: Some("NYC".to_string()) };
        ...
        assert_eq!(result.summary.points_inside + result.summary.points_outside, 5);
    ...
    fn test_proximity_zone_all_inside() {
        let center = Point { lat: 40.7128, lon: -74.0060, id: None };
    ...
    fn test_proximity_zone_all_outside() {
        ...
        let candidates = vec![
            Point { lat: 40.8000, lon: -74.0060, id: Some("Far1".to_string()) },
            Point { lat: 41.0000, lon: -74.0060, id: Some("Far2".to_string()) },
        ];
    ...
    fn test_proximity_zone_bearings() {
        ...
        let radius = 10000.0; // 10km
        let candidates = vec![
            Point { lat: 41.0, lon: -74.0, id: Some("North".to_string()) },   // North
            Point { lat: 40.0, lon: -73.0, id: Some("East".to_string()) },    // East
            Point { lat: 39.0, lon: -74.0, id: Some("South".to_string()) },   // South
            Point { lat: 40.0, lon: -75.0, id: Some("West".to_string()) },    // West
        ];
    ...
    fn test_haversine_distance_calculation() {
        let p1 = Point { lat: 40.7128, lon: -74.0060, id: None }; // NYC
        let p2 = Point { lat: 34.0522, lon: -118.2437, id: None }; // LA
    ...
    fn test_proximity_zone_invalid_center_coordinates() {
        ...
        // Invalid latitude
        let invalid_center = Point { lat: 91.0, lon: -74.0, id: None };
        ...
        // Invalid longitude
        let invalid_center = Point { lat: 40.0, lon: 181.0, id: None };
    ...
    fn test_proximity_zone_nan_coordinates() {
        ...
        let nan_center = Point { lat: f64::NAN, lon: -74.0, id: None };
        let result = proximity_zone_analysis(nan_center, 1000.0, candidates.clone());
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Center latitude cannot be NaN or infinite");
    ...
    fn test_proximity_zone_infinite_coordinates() {
        ...
        let inf_center = Point { lat: f64::INFINITY, lon: -74.0, id: None };
    ...
    fn test_proximity_zone_boundary_coordinates() {
        ...
        let center = Point { lat: 90.0, lon: 180.0, id: None }; // North Pole, Date Line
        let candidates = vec![
    ...
    fn test_proximity_zone_exact_radius_boundary() {
        ...
        let candidates = vec![
            Point { lat: 40.009, lon: -74.0, id: Some("AtRadius".to_string()) }, // ~1km north
        ];
    ...
    fn test_proximity_zone_large_dataset() {
        ...
        assert!(result.summary.points_inside > 0);
        assert!(result.summary.points_outside >= 0);
        assert_eq!(result.summary.points_inside + result.summary.points_outside, 100);
    }

    #[test]
    fn test_proximity_zone_point_ids_preserved() {
        let center = Point { lat: 40.0, lon: -74.0, id: Some("Center".to_string()) };
        let radius = 10000.0; // 10km
        let candidates = create_test_points_around_center();
        ...
        let all_points: Vec<&NearestPointResult> = result.points_in_zone.iter()
