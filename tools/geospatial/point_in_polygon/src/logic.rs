...
pub fn point_in_polygon_check(point: Point, polygon: Vec<Point>) -> Result<PointInPolygonResult, String> {
    if polygon.len() < 3 {
...
mod tests {
    ...
    fn test_ray_casting_algorithm_simple() {
        ...
        assert!(ray_casting_algorithm(&Point { lat: 0.5, lon: 0.5 }, &square));
        assert!(!ray_casting_algorithm(&Point { lat: 2.0, lon: 2.0 }, &square));
        assert!(!ray_casting_algorithm(&Point { lat: -1.0, lon: -1.0 }, &square));
    }
    ...
    fn test_ray_casting_complex_polygon() {
        ...
        assert!(ray_casting_algorithm(&Point { lat: 0.5, lon: 0.5 }, &l_shape));
        assert!(ray_casting_algorithm(&Point { lat: 0.5, lon: 2.0 }, &l_shape));
        assert!(!ray_casting_algorithm(&Point { lat: 1.5, lon: 2.0 }, &l_shape));
        assert!(!ray_casting_algorithm(&Point { lat: 3.0, lon: 1.5 }, &l_shape)); // Clearly outside
    }
    ...
    fn test_is_point_on_segment() {
        ...
        // Point on segment
        assert!(is_point_on_segment(&Point { lat: 0.5, lon: 0.5 }, &start, &end));
        
        // Point at start
        assert!(is_point_on_segment(&Point { lat: 0.0, lon: 0.0 }, &start, &end));
        
        // Point at end
        assert!(is_point_on_segment(&Point { lat: 1.0, lon: 1.0 }, &start, &end));
        
        // Point not on segment
        assert!(!is_point_on_segment(&Point { lat: 0.5, lon: 0.6 }, &start, &end));
        
        // Point on line but outside segment
        assert!(!is_point_on_segment(&Point { lat: 2.0, lon: 2.0 }, &start, &end));
        assert!(!is_point_on_segment(&Point { lat: -0.5, lon: -0.5 }, &start, &end));
    }
    ...
    fn test_point_in_polygon_nan_coordinates() {
        ...
        let nan_point = Point { lat: f64::NAN, lon: 0.0 };
        let result = point_in_polygon_check(nan_point, square.clone());
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Point latitude cannot be NaN or infinite");
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Polygon vertex longitude cannot be NaN or infinite");
    }
    ...
    fn test_point_in_polygon_infinite_coordinates() {
        ...
        let inf_point = Point { lat: f64::INFINITY, lon: 0.0 };
        ...
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Polygon vertex latitude cannot be NaN or infinite");
    }
    ...
    fn test_point_in_polygon_real_world_coordinates() {
        ...
        let manhattan = vec![
            Point { lat: 40.700, lon: -74.025 },
            Point { lat: 40.700, lon: -73.930 },
            Point { lat: 40.820, lon: -73.930 },
            Point { lat: 40.820, lon: -74.025 },
        ];
        
        // Point in Times Square
        let times_square = Point { lat: 40.758, lon: -73.985 };
        let result = point_in_polygon_check(times_square, manhattan.clone()).unwrap();
        assert!(result.is_inside);
        
        // Point in Brooklyn (outside)
        let brooklyn = Point { lat: 40.650, lon: -73.950 };
    ...
    fn test_point_in_polygon_edge_cases() {
        ...
        let near_boundary = Point { lat: 0.0000001, lon: 0.5 };
        let result = point_in_polygon_check(near_boundary, square.clone()).unwrap();
        assert!(result.is_inside);
        assert!(!result.on_boundary);
        
        // Point just outside
        let just_outside = Point { lat: -0.0000001, lon: 0.5 };
