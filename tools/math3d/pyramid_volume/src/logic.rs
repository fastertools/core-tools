...
mod tests {
    ...
    fn test_triangular_pyramid() {
        let input = PyramidInput {
            base_points: vec![
                ...
                Vector3D { x: 2.0, y: 0.0, z: 0.0 },
                Vector3D { x: 1.0, y: 2.0, z: 0.0 },
            ],
            apex: Vector3D { x: 1.0, y: 1.0, z: 3.0 },
        };
    ...
    fn test_unit_cube_pyramid() {
        let input = PyramidInput {
            base_points: vec![
                ...
                Vector3D { x: 1.0, y: 1.0, z: 0.0 },
                Vector3D { x: 0.0, y: 1.0, z: 0.0 },
            ],
            apex: Vector3D { x: 0.5, y: 0.5, z: 1.0 },
        };
        let result = compute_pyramid_volume(input).unwrap();
    ...
    fn test_pentagon_pyramid() {
        let input = PyramidInput {
            base_points: vec![
                Vector3D { x: 1.0, y: 0.0, z: 0.0 },
                Vector3D { x: 0.309, y: 0.951, z: 0.0 },
                Vector3D { x: -0.809, y: 0.588, z: 0.0 },
                Vector3D { x: -0.809, y: -0.588, z: 0.0 },
                Vector3D { x: 0.309, y: -0.951, z: 0.0 },
            ],
            apex: Vector3D { x: 0.0, y: 0.0, z: 2.0 },
        };
        let result = compute_pyramid_volume(input).unwrap();
    ...
    fn test_zero_height_pyramid() {
        let input = PyramidInput {
            ...
            apex: Vector3D { x: 0.5, y: 0.5, z: 0.0 }, // Apex in same plane as base
        };
        let result = compute_pyramid_volume(input).unwrap();
    ...
    fn test_large_coordinates() {
        let input = PyramidInput {
            base_points: vec![
                Vector3D { x: 1000.0, y: 1000.0, z: 1000.0 },
                Vector3D { x: 1001.0, y: 1000.0, z: 1000.0 },
                Vector3D { x: 1000.0, y: 1001.0, z: 1000.0 },
            ],
            apex: Vector3D { x: 1000.5, y: 1000.5, z: 1001.0 },
        };
        let result = compute_pyramid_volume(input).unwrap();
    ...
    fn test_calculation_method_field() {
        let input = PyramidInput {
            ...
            apex: Vector3D { x: 0.0, y: 0.0, z: 1.0 },
        };
        let result = compute_pyramid_volume(input).unwrap();
        assert_eq!(result.calculation_method, "Pyramid formula: (1/3) × base_area × height");
    }
    ...
    fn test_insufficient_base_points_error() {
        ...
        assert_eq!(result.unwrap_err(), "At least 3 points are required for the base");
    }
    ...
    fn test_nan_apex_error() {
        let input = PyramidInput {
            ...
            apex: Vector3D { x: f64::NAN, y: 0.0, z: 1.0 },
        };
        let result = compute_pyramid_volume(input);
        assert!(result.is_err());
    ...
    fn test_infinite_apex_error() {
        let input = PyramidInput {
            ...
            apex: Vector3D { x: 0.0, y: f64::INFINITY, z: 1.0 },
        };
        let result = compute_pyramid_volume(input);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("Apex"));
    ...
    fn test_nan_base_point_error() {
        let input = PyramidInput {
            base_points: vec![
                Vector3D { x: f64::NAN, y: 0.0, z: 0.0 },
                Vector3D { x: 1.0, y: 0.0, z: 0.0 },
                Vector3D { x: 0.0, y: 1.0, z: 0.0 },
            ],
            apex: Vector3D { x: 0.0, y: 0.0, z: 1.0 },
    ...
    fn test_infinite_base_point_error() {
        let input = PyramidInput {
            base_points: vec![
                Vector3D { x: 0.0, y: 0.0, z: 0.0 },
                Vector3D { x: f64::INFINITY, y: 0.0, z: 0.0 },
                Vector3D { x: 0.0, y: 1.0, z: 0.0 },
            ],
            apex: Vector3D { x: 0.0, y: 0.0, z: 1.0 },
        };
