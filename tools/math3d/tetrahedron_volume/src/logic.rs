...
pub fn compute_tetrahedron_volume(
    input: TetrahedronVolumeInput,
) -> Result<TetrahedronVolumeResponse, String> {
    ...
    for (i, point) in points.iter().enumerate() {
        ...
        if point.x.is_infinite() || point.y.is_infinite() || point.z.is_infinite() {
            return Err(format!(
                "Point {} contains infinite values",
                ['A', 'B', 'C', 'D'][i]
            ));
        }
    }
...
mod tests {
    ...
    fn test_regular_tetrahedron() {
        ...
        let input = TetrahedronVolumeInput {
            ...
            point_c: Vector3D {
                x: edge_length / 2.0,
                y: edge_length * 3.0_f64.sqrt() / 2.0,
                z: 0.0,
            },
            point_d: Vector3D {
                x: edge_length / 2.0,
                y: edge_length * 3.0_f64.sqrt() / 6.0,
                z: height,
            },
        };
        let result = compute_tetrahedron_volume(input).unwrap();
