...
impl Vector3D {
    ...
    pub fn to_spherical(&self) -> SphericalCoord {
        ...
        let phi = if radius > 0.0 { (self.z / radius).acos() } else { 0.0 };
    ...
    pub fn to_cylindrical(&self) -> CylindricalCoord {
        ...
        CylindricalCoord { radius, theta, z: self.z }
    }
}
...
pub fn coordinate_conversion_logic(input: CoordinateConversionInput) -> Result<CoordinateConversionOutput, String> {
    // Input validation
    ...
    let converted = match (from_type.as_str(), to_type.as_str()) {
        ("cartesian", "spherical") => {
            ...
            if !spherical.is_valid() {
                return Err("Conversion to spherical coordinates resulted in invalid values".to_string());
            }
            Vector3D {
                x: spherical.radius,
                y: spherical.theta,
        ...
        ("spherical", "cartesian") => {
            ...
            if !spherical.is_valid() {
                return Err("Invalid spherical coordinates: radius must be non-negative".to_string());
            }
            let cartesian = spherical.to_cartesian();
            if !cartesian.is_valid() {
                return Err("Conversion from spherical coordinates resulted in invalid values".to_string());
            }
            cartesian
        },
        ...
        ("cylindrical", "cartesian") => {
            ...
            if !cylindrical.is_valid() {
                return Err("Invalid cylindrical coordinates: radius must be non-negative".to_string());
            }
            let cartesian = cylindrical.to_cartesian();
            if !cartesian.is_valid() {
                return Err("Conversion from cylindrical coordinates resulted in invalid values".to_string());
            }
            cartesian
        },
        _ => {
...
mod tests {
    ...
    fn test_cartesian_to_spherical() {
        let cartesian = Vector3D { x: 1.0, y: 0.0, z: 0.0 };
    ...
    fn test_cartesian_to_cylindrical() {
        let cartesian = Vector3D { x: 1.0, y: 0.0, z: 2.0 };
        let input = CoordinateConversionInput {
            from_type: "cartesian".to_string(),
    ...
    fn test_round_trip_cartesian_spherical() {
        let original = Vector3D { x: 3.0, y: 4.0, z: 5.0 };
    ...
    fn test_invalid_conversion_type() {
        let input = CoordinateConversionInput {
            ...
            to_type: "cartesian".to_string(),
            coordinates: Vector3D { x: 1.0, y: 1.0, z: 1.0 },
        };
        
        let result = coordinate_conversion_logic(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Invalid coordinate conversion. Supported: cartesian↔spherical, cartesian↔cylindrical");
    }
    ...
    fn test_case_insensitive_conversion_types() {
        let input = CoordinateConversionInput {
            from_type: "CARTESIAN".to_string(),
            to_type: "Spherical".to_string(),
            coordinates: Vector3D { x: 1.0, y: 0.0, z: 0.0 },
        };
    ...
    fn test_nan_coordinates() {
        let input = CoordinateConversionInput {
            from_type: "cartesian".to_string(),
            to_type: "spherical".to_string(),
            coordinates: Vector3D { x: f64::NAN, y: 0.0, z: 0.0 },
        };
        
        let result = coordinate_conversion_logic(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Invalid coordinates: contains NaN or infinite values");
    }
    ...
    fn test_infinite_coordinates() {
        let input = CoordinateConversionInput {
            from_type: "cartesian".to_string(),
            to_type: "spherical".to_string(),
            coordinates: Vector3D { x: f64::INFINITY, y: 0.0, z: 0.0 },
        };
    ...
    fn test_negative_spherical_radius() {
        let input = CoordinateConversionInput {
            from_type: "spherical".to_string(),
            to_type: "cartesian".to_string(),
            coordinates: Vector3D { x: -1.0, y: 0.0, z: 0.0 },  // negative radius
        };
        
        let result = coordinate_conversion_logic(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Invalid spherical coordinates: radius must be non-negative");
    }
    ...
    fn test_negative_cylindrical_radius() {
        let input = CoordinateConversionInput {
            from_type: "cylindrical".to_string(),
            to_type: "cartesian".to_string(),
            coordinates: Vector3D { x: -1.0, y: 0.0, z: 1.0 },  // negative radius
        };
        
        let result = coordinate_conversion_logic(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Invalid cylindrical coordinates: radius must be non-negative");
    }
    ...
    fn test_coordinate_validation() {
        ...
        let valid_spherical = SphericalCoord { radius: 1.0, theta: 0.0, phi: 0.0 };
        assert!(valid_spherical.is_valid());
        
        let invalid_spherical = SphericalCoord { radius: -1.0, theta: 0.0, phi: 0.0 };
        assert!(!invalid_spherical.is_valid());
        
        let valid_cylindrical = CylindricalCoord { radius: 1.0, theta: 0.0, z: 1.0 };
        assert!(valid_cylindrical.is_valid());
        
        let invalid_cylindrical = CylindricalCoord { radius: -1.0, theta: 0.0, z: 1.0 };
    ...
    fn test_specific_angle_conversions() {
        ...
        let test_cases = vec![
            ...
            (0.0, 1.0, 0.0, 1.0, std::f64::consts::PI / 2.0, std::f64::consts::PI / 2.0), // +Y axis
        ...
        for (x, y, z, expected_r, expected_theta, expected_phi) in test_cases {
            ...
            let result = coordinate_conversion_logic(input).unwrap();
            assert!((result.converted.x - expected_r).abs() < 1e-14, 
                    "Radius mismatch for ({}, {}, {})", x, y, z);
            // Note: theta can vary for points on z-axis, so we only check it for off-axis points
            if x != 0.0 || y != 0.0 {
                assert!((result.converted.y - expected_theta).abs() < 1e-14,
                        "Theta mismatch for ({}, {}, {})", x, y, z);
            }
            assert!((result.converted.z - expected_phi).abs() < 1e-14,
