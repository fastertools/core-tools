...
pub fn compute_quaternion_multiply(input: QuaternionMultiplyInput) -> Result<QuaternionMultiplyResponse, String> {
    ...
    if input.q1.x.is_infinite() || input.q1.y.is_infinite() || input.q1.z.is_infinite() || input.q1.w.is_infinite() {
    ...
    if input.q2.x.is_infinite() || input.q2.y.is_infinite() || input.q2.z.is_infinite() || input.q2.w.is_infinite() {
...
mod tests {
    ...
    fn test_identity_multiplication() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 0.0, y: 0.0, z: 0.0, w: 1.0 }, // Identity
    ...
    fn test_multiplication_by_identity() {
        let input = QuaternionMultiplyInput {
            ...
            q2: Quaternion { x: 0.0, y: 0.0, z: 0.0, w: 1.0 }, // Identity
        };
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 1.0, y: 2.0, z: 3.0, w: 4.0 };
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    ...
    fn test_zero_quaternion_multiplication() {
        ...
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 0.0, y: 0.0, z: 0.0, w: 0.0 };
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_i_j_multiplication() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 }, // i
            q2: Quaternion { x: 0.0, y: 1.0, z: 0.0, w: 0.0 }, // j
        };
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 0.0, y: 0.0, z: 1.0, w: 0.0 }; // k
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_j_k_multiplication() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 0.0, y: 1.0, z: 0.0, w: 0.0 }, // j
            q2: Quaternion { x: 0.0, y: 0.0, z: 1.0, w: 0.0 }, // k
        };
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 }; // i
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_k_i_multiplication() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 0.0, y: 0.0, z: 1.0, w: 0.0 }, // k
            q2: Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 }, // i
        };
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 0.0, y: 1.0, z: 0.0, w: 0.0 }; // j
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_i_squared() {
        ...
        let result = compute_quaternion_multiply(input).unwrap();
        let expected = Quaternion { x: 0.0, y: 0.0, z: 0.0, w: -1.0 }; // -1
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_general_multiplication() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 1.0, y: 2.0, z: 3.0, w: 4.0 },
            q2: Quaternion { x: 5.0, y: 6.0, z: 7.0, w: 8.0 },
        };
        let result = compute_quaternion_multiply(input).unwrap();
        // Calculated manually: 
        ...
        let expected = Quaternion { x: 24.0, y: 48.0, z: 48.0, w: -6.0 };
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }

    #[test]
    fn test_multiplication_non_commutative() {
        let q1 = Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 }; // i
        let q2 = Quaternion { x: 0.0, y: 1.0, z: 0.0, w: 0.0 }; // j
        
        let input1 = QuaternionMultiplyInput { q1: q1.clone(), q2: q2.clone() };
        ...
        // i * j = k, but j * i = -k
        assert_quaternion_eq(&result1.result, &Quaternion { x: 0.0, y: 0.0, z: 1.0, w: 0.0 }, 1e-15);
        assert_quaternion_eq(&result2.result, &Quaternion { x: 0.0, y: 0.0, z: -1.0, w: 0.0 }, 1e-15);
    }
    ...
    fn test_negative_values() {
        ...
        let expected = Quaternion { x: -8.0, y: -16.0, z: -24.0, w: -2.0 };
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_unit_quaternion_multiplication() {
        ...
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: sqrt2_inv, y: 0.0, z: 0.0, w: sqrt2_inv }, // 90° around X
            q2: Quaternion { x: 0.0, y: sqrt2_inv, z: 0.0, w: sqrt2_inv }, // 90° around Y
        };
        let result = compute_quaternion_multiply(input).unwrap();
        
        // Result should still be a unit quaternion
        let magnitude_squared = result.result.x.powi(2) + result.result.y.powi(2) + 
                               result.result.z.powi(2) + result.result.w.powi(2);
    ...
    fn test_fractional_values() {
        let input = QuaternionMultiplyInput {
            ...
            q2: Quaternion { x: 0.5, y: -0.5, z: 0.5, w: -0.5 },
        };
        let result = compute_quaternion_multiply(input).unwrap();
        // Calculated manually:
        ...
        let expected = Quaternion { x: 0.5, y: -0.5, z: -0.5, w: -0.5 };
        assert_quaternion_eq(&result.result, &expected, 1e-15);
    }
    ...
    fn test_nan_q1_error() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: f64::NAN, y: 0.0, z: 0.0, w: 1.0 },
            q2: Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 },
        };
    ...
    fn test_infinite_q2_error() {
        let input = QuaternionMultiplyInput {
            q1: Quaternion { x: 1.0, y: 0.0, z: 0.0, w: 0.0 },
            q2: Quaternion { x: 0.0, y: f64::INFINITY, z: 0.0, w: 1.0 },
        };
        let result = compute_quaternion_multiply(input);
        assert!(result.is_err());
