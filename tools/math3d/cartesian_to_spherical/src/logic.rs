...
pub fn cartesian_to_spherical_logic(input: CartesianToSphericalInput) -> Result<CartesianToSphericalOutput, String> {
    // Input validation
...
mod tests {
    ...
    #[test]
    fn test_origin() {
        let cartesian = Vector3D { x: 0.0, y: 0.0, z: 0.0 };
    ...
    fn test_positive_y_axis() {
        let cartesian = Vector3D { x: 0.0, y: 1.0, z: 0.0 };
    ...
    fn test_positive_z_axis() {
        let cartesian = Vector3D { x: 0.0, y: 0.0, z: 1.0 };
    ...
    fn test_arbitrary_point() {
        let cartesian = Vector3D { x: 3.0, y: 4.0, z: 5.0 };
    ...
    fn test_round_trip_conversion() {
        ...
        for original in original_points {
            ...
            assert!((converted_back.x - original.x).abs() < 1e-14, 
                    "X mismatch: {} vs {}", converted_back.x, original.x);
            assert!((converted_back.y - original.y).abs() < 1e-14,
                    "Y mismatch: {} vs {}", converted_back.y, original.y);
            assert!((converted_back.z - original.z).abs() < 1e-14,
                    "Z mismatch: {} vs {}", converted_back.z, original.z);
        }
    }

    #[test]
    fn test_nan_coordinates() {
        let cartesian = Vector3D { x: f64::NAN, y: 0.0, z: 0.0 };
        ...
        assert_eq!(result.unwrap_err(), "Invalid Cartesian coordinates: contains NaN or infinite values");
    }

    #[test]
    fn test_infinite_coordinates() {
        let cartesian = Vector3D { x: f64::INFINITY, y: 0.0, z: 0.0 };
    ...
    fn test_large_coordinates() {
        let cartesian = Vector3D { x: 1e10, y: 0.0, z: 0.0 };
    ...
    fn test_negative_coordinates() {
        let cartesian = Vector3D { x: -1.0, y: -1.0, z: -1.0 };
    ...
    fn test_vector_validation() {
        ...
        let infinite_vector = Vector3D { x: f64::INFINITY, y: 2.0, z: 3.0 };
    ...
    fn test_conversion_notes() {
        let cartesian = Vector3D { x: 1.0, y: 2.0, z: 3.0 };
    ...
    fn test_vector_magnitude() {
        ...
        let zero_vector = Vector3D { x: 0.0, y: 0.0, z: 0.0 };
    ...
    fn test_quadrant_angles() {
        ...
        let test_cases = vec![
            (Vector3D { x: 1.0, y: 1.0, z: 0.0 }, std::f64::consts::PI / 4.0),      // First quadrant
            (Vector3D { x: -1.0, y: 1.0, z: 0.0 }, 3.0 * std::f64::consts::PI / 4.0), // Second quadrant
            (Vector3D { x: -1.0, y: -1.0, z: 0.0 }, -3.0 * std::f64::consts::PI / 4.0), // Third quadrant
            (Vector3D { x: 1.0, y: -1.0, z: 0.0 }, -std::f64::consts::PI / 4.0),     // Fourth quadrant
        ];
        
        for (cartesian, expected_theta) in test_cases {
            let input = CartesianToSphericalInput { coordinates: cartesian.clone() };
            let result = cartesian_to_spherical_logic(input).unwrap();
            
            assert!((result.spherical_coordinates.theta - expected_theta).abs() < 1e-14,
                    "Theta mismatch for ({}, {}): expected {}, got {}",
                    cartesian.x, cartesian.y, expected_theta, result.spherical_coordinates.theta);
        }
    }
}