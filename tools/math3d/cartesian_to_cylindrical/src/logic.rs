...
pub fn cartesian_to_cylindrical_logic(input: CartesianCoordinates) -> Result<CartesianToCylindricalResult, String> {
    // Input validation
    if !input.is_valid() {
    ...
    let conversion_notes = format!(
        ...
        cylindrical.radius, cylindrical.theta, cylindrical.z
    );
...
mod tests {
    ...
    fn test_negative_coordinates() {
        ...
        let result = cartesian_to_cylindrical_logic(input).unwrap();
        assert!((result.cylindrical_coordinates.radius - 2.0_f64.sqrt()).abs() < 1e-15);
        assert!((result.cylindrical_coordinates.theta - (-3.0 * std::f64::consts::PI / 4.0)).abs() < 1e-15);
        assert!((result.cylindrical_coordinates.z - (-2.0)).abs() < 1e-15);
    }
    ...
    fn test_cylindrical_validation() {
        ...
        let invalid_negative_radius = CylindricalCoordinates { radius: -1.0, theta: 0.0, z: 1.0 };
    ...
    fn test_specific_angles() {
        ...
        for (x, y, z, expected_radius, expected_theta) in test_cases {
            ...
            assert!((result.cylindrical_coordinates.radius - expected_radius).abs() < 1e-14,
                    "Radius mismatch for ({}, {}, {})", x, y, z);
            assert!((result.cylindrical_coordinates.theta - expected_theta).abs() < 1e-14,
                    "Theta mismatch for ({}, {}, {})", x, y, z);
            assert!((result.cylindrical_coordinates.z - z).abs() < 1e-14,
