    ...
    let processed = match input.operation.as_str() {
        ...
        "trim_char_start" => {
            if let Some(ch) = input.char_to_trim.as_ref().and_then(|s| s.chars().next()) {
                ...
                return Err("char_to_trim must be provided for trim_char_start operation".to_string());
            }
        },
        ...
        "pad" | "pad_left" | "pad_right" | "pad_center" => {
            let pad_length = input.pad_length.ok_or("pad_length must be provided for padding operations")?;
        ...
        _ => return Err(format!("Unknown operation: {}. Valid operations: trim, trim_start, trim_end, trim_char, trim_char_start, trim_char_end, pad, pad_left, pad_right, pad_center", input.operation)),
    };
