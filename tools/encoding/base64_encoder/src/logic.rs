...
pub fn encode_base64(input: Base64EncoderInput) -> Result<Base64EncoderOutput, String> {
    ...
    let encoded = match variant.as_str() {
        "standard" => {
            general_purpose::STANDARD.encode(&input.data)
        },
        "standard_no_pad" => {
            general_purpose::STANDARD_NO_PAD.encode(&input.data)
        },
        "url_safe" => {
            general_purpose::URL_SAFE.encode(&input.data)
        },
        "url_safe_no_pad" => {
            general_purpose::URL_SAFE_NO_PAD.encode(&input.data)
        },
        _ => {
            return Err(format!(
