...
pub async fn calculate_analyze_distribution(input: AnalyzeDistributionInput) -> Result<AnalyzeDistributionOutput, String> {
    ...
    let distribution_parameters = calculate_distribution_parameters(&input.data, normality_test.is_normal)?;
...
async fn call_histogram_tool(data: &[f64], num_bins: Option<usize>) -> Result<HistogramOutput, String> {
    ...
    let wrapper: ToolResponseWrapper<HistogramOutput> = serde_json::from_str(&body)
...
async fn call_test_normality_tool(data: &[f64]) -> Result<NormalityTestOutput, String> {
    ...
    let wrapper: ToolResponseWrapper<NormalityTestOutput> = serde_json::from_str(&body)
        .map_err(|e| format!("Failed to parse tool response: {}", e))?;
...
fn calculate_distribution_parameters(data: &[f64], is_normal: bool) -> Result<DistributionParameters, String> {
    ...
    if std_dev == 0.0 {
        return Err("Standard deviation is zero, cannot calculate distribution parameters".to_string());
    }
...
mod tests {
    ...
    fn test_suggest_distribution() {
        ...
        assert_eq!(
            suggest_distribution(0.0, 0.0, true),
            "Normal Distribution"
        );
    ...
    fn test_skewed_data() {
        ...
        assert!(result.suggested_distribution.contains("Right-skewed") || 
