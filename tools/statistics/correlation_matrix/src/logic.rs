...
mod tests {
    ...
    fn test_three_variable_matrix() {
        ...
        for i in 0..3 {
            for j in 0..3 {
                assert!((result.correlation_matrix[i][j] - result.correlation_matrix[j][i]).abs() < 0.0001);
            }
        }
    }
    ...
    fn test_mismatched_lengths_error() {
        ...
        let result = calculate_correlation_matrix(input);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("All data series must have the same length"));
    }
    ...
    fn test_mismatched_variable_names_error() {
        ...
        let result = calculate_correlation_matrix(input);
        assert!(result.is_err());
        assert_eq!(result.unwrap_err(), "Number of variable names must match number of data series");
    }
